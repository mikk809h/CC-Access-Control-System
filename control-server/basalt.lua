...
require = function(path)
    if (project[path .. ".lua"]) then
        if (loadedProject[path] == nil) then loadedProject[path] = project[path .. ".lua"]() end
...
project["plugins/canvas.lua"] = function(...)
    ...
    local da = {}
    da.__index = da; local _b, ab = string.sub, string.rep
    function da.new(cb)
        local db = setmetatable({}, da)
        db.commands = { pre = {}, post = {} }
        db.type = "pre"
        db.element = cb; return db
    end

    function da:clear()
        self.commands = { pre = {}, post = {} }
        return self
    end; function da:getValue(cb)
        if type(cb) == "function" then return cb(self.element) end; return cb
    end

    function da:setType(cb)
        if
            cb == "pre" or cb == "post" then
            self.type = cb
        else
    ...
    function da:addCommand(cb)
        local db = #self.commands[self.type] + 1; self.commands[self.type][db] = cb; return db
    end

    function da:setCommand(cb, db)
        self.commands[cb] = db; return self
    end; function da:removeCommand(cb)
    ...
    function da:text(cb, db, _c, ac, bc)
        return
            self:addCommand(function(cc)
                ...
                if dd then cc:drawFg(dc, _d, dd) end; if __a then cc:drawBg(dc, _d, __a) end
            end)
    end; function da:bg(cb, db, _c)
        return
            self:addCommand(function(ac) ac:drawBg(cb, db, _c) end)
    end
    ...
    function da:rect(cb, db, _c, ac, bc, cc, dc)
        return
            self:addCommand(function(_d)
                ...
                if (type(a_a) == "number") then a_a = ba[a_a] end; if
                    (type(b_a) == "number") then
                    b_a = ba[b_a]
                end
        ...
        return
            self:addCommand(function(cd)
                ...
                ipairs(bd) do
                    ...
                    end; if b_a then cd:drawFg(aaa, baa, b_a) end; if c_a then
    ...
    function da:ellipse(cb, db, _c, ac, bc, cc, dc)
        local function _d(bd, cd, dd, __a)
            local a_a = {}
            local b_a = 0; local c_a = dd * dd; local d_a = __a * __a; local _aa = 0; local aaa = __a; local baa = d_a -
                c_a * __a +
            ...
            local function _ba(aba, bba)
                ...
                b_a = b_a + 1
                a_a[b_a] = { x = bd - aba, y = cd + bba }
                b_a = b_a + 1; a_a[b_a] = { x = bd + aba, y = cd - bba }
                b_a = b_a + 1; a_a[b_a] = {
                    x = bd - aba,
                    y =
                        cd - bba
            ...
            while caa < daa do
                _aa = _aa + 1; caa = caa + 2 * d_a
                if baa < 0 then
                    baa = baa + d_a + caa
                else
                    aaa = aaa - 1; daa = daa - 2 * c_a; baa = baa + d_a + caa - daa
                end; _ba(_aa, aaa)
            end; baa =
                d_a * (_aa + 0.5) * (_aa + 0.5) + c_a * (aaa - 1) * (aaa - 1) - c_a * d_a; while aaa > 0 do
                aaa = aaa - 1; daa = daa - 2 * c_a; if baa > 0 then
                    baa = baa + c_a - daa
                else
                    _aa = _aa + 1
                    caa = caa + 2 * d_a; baa = baa + c_a - daa + caa
                end
                _ba(_aa, aaa)
            end
            return a_a
        end; local ad = _d(cb, db, _c, ac)
        return
            self:addCommand(function(bd)
                local cd = self:getValue(bc)
                ...
                for c_a, d_a in pairs(ad) do
                    ...
                    end; if a_a then bd:drawFg(_aa, aaa, a_a) end; if b_a then
    ...
    function bb.setup(cb)
        cb.defineProperty(cb, "canvas", {
            ...
            getter = function(db)
                ...
                end; return db._values.canvas
            end
        })
    end; function bb.hooks.render(cb)
        local db = cb.get("canvas")
        if
            db and #db.commands.pre > 0 then
            for _c, ac in pairs(db.commands.pre) do ac(cb) end
        end
    end
    ...
    end; return { VisualElement = bb, API = da }
end
project["plugins/debug.lua"] = function(...)
    ...
    local function _c(dc)
        ...
        return
        {
            trackProperty = function(ad, bd) _d.properties[ad] = bd end,
            trackRender = function()
                _d.renderCount = _d.renderCount + 1; _d.lastRender = os.epoch("utc")
            end,
            trackEvent = function(ad)
                _d.eventCount[ad] = (
                    _d.eventCount[ad] or 0) + 1
            end,
            dump = function()
                return
                { type = dc.get("type"), id = dc.get("id"), stats = _d }
            end
        }
    end; local ac = {}
    function ac.debug(dc, _d)
        dc._debugger = _c(dc)
        dc._debugLevel = _d or db.INFO; return dc
    end; function ac.dumpDebug(dc)
        if not dc._debugger then return end
        return dc._debugger.dump()
    end; local bc = {}
    function bc.openConsole(dc)
        if
            not dc._debugFrame then
            ...
            local function cd()
                ...
                ipairs(_b._logs) do
                    ...
                    for baa, caa in ipairs(aaa) do
                        table.insert(b_a, { text = caa, level = _aa.level })
                    end
                end; return b_a
            end; local dd = #cd() - dc.get("height")
            dc._scrollOffset = dd
                ...
                for i = 1, c_a - 2 do
                    ...
                    if baa then
                        local caa =
                            ...
                            _b.LEVEL.WARN and colors.yellow or baa.level == _b.LEVEL.DEBUG and colors.lightGray or
                            colors.white; b_a:textFg(2, i, baa.text, caa)
                    end
                end
            end; local a_a = dc._debugFrame.dispatchEvent
    ...
    end; function bc.toggleConsole(dc)
    ...
    end; return { BaseElement = ac, Container = cc, BaseFrame = bc }
end
project["plugins/reactive.lua"] = function(...)
    local ab = require("errorManager")
    local bb = require("propertySystem")
    local cb = { colors = true, math = true, clamp = true, round = true }
    local db = {
        clamp = function(ad, bd, cd)
            return
                math.min(math.max(ad, bd), cd)
        end,
        round = function(ad)
    ...
    local function _c(ad, bd, cd)
        ...
        ad = ad:gsub("([%w_]+)%$([%w_]+)", function(b_a, c_a)
            if b_a == "self" then
                return
                    string.format('__getState("%s")', c_a)
            elseif b_a == "parent" then
                return
                    string.format('__getParentState("%s")', c_a)
            else
                return
                    string.format('__getElementState("%s", "%s")', b_a, c_a)
            end
        end)
        ad = ad:gsub("([%w_]+)%.([%w_]+)", function(b_a, c_a)
            if cb[b_a] then return b_a .. "." .. c_a end; if
                tonumber(b_a) then
                return b_a .. "." .. c_a
            end; return
                string.format('__getProperty("%s", "%s")', b_a, c_a)
        end)
        local dd = setmetatable(
            {
                ...
                tostring = tostring,
                tonumber = tonumber,
                __getState = function(b_a) return bd:getState(b_a) end,
                __getParentState = function(b_a)
                    return
                        bd.parent:getState(b_a)
                end,
                __getElementState = function(b_a, c_a)
                    if tonumber(b_a) then return nil end
    ...
    end; local bc = setmetatable({}, { __mode = "k" })
    ...
    bb.addSetterHook(function(ad, bd, cd, dd)
        if type(cd) == "string" and cd:match("^{.+}$") then
            local __a = cd:gsub("^{(.+)}$", "%1")
            if not ac(__a, ad) then return dd.default end; dc(ad, __a, bd)
            if
                not bc[ad] then
                bc[ad] = {}
            end; if not bc[ad][cd] then
...
project["plugins/theme.lua"] = function(...)
    ...
    local _c = {
        hooks = {
            postInit = {
                pre = function(ad)
                    if ad._postInitialized then return ad end
                    ad:applyTheme()
                end
            }
        }
    ...
    function _c.____getElementPath(ad, bd)
        if bd then
            ...
            bd = { ad._values.type }
        end; local cd = ad.parent; if cd then
            return
                cd.____getElementPath(cd, bd)
        else
            return bd
        end
    end
    ...
    local function ac(ad, bd)
        local cd = ad
        for i = 1, #bd do
            local dd = false; local __a = bd[i]
            for a_a, b_a in ipairs(__a) do
                if cd[b_a] then
                    cd = cd[b_a]
                    dd = true; break
                end
            end; if not dd then return nil end
        end; return cd
    end
    local function bc(ad, bd)
        ...
        if ad.default then
            for dd, __a in pairs(ad.default) do
                if type(__a) ~= "table" then cd[dd] = __a end
            end; if ad.default[bd] then
                for dd, __a in
                pairs(ad.default[bd]) do if type(__a) ~= "table" then cd[dd] = __a end end
            end
        end; return cd
    end
    local function cc(ad, bd, cd, dd, __a)
        if
            bd.default and bd.default.names and bd.default.names[dd] then
            for a_a, b_a in pairs(bd.default.names[dd]) do
                if type(b_a) ~= "table" then ad[a_a] = b_a end
            end
        end
        if
        ...
        end; if __a and __a.names and __a.names[dd] then
    ...
    local function dc(ad, bd, cd, dd)
        ...
        if a_a then
            for b_a, c_a in pairs(a_a) do
                if type(c_a) ~= "table" then __a[b_a] = c_a end
            end
        end; if next(__a) == nil then __a = bc(ad, cd) end
        cc(__a, ad, cd, dd, a_a)
        return __a
    end
    function _c:applyTheme(ad)
                ...
                if (__a) then
                    if ((__a.type) == "color") then
                        if (type(dd) == "string") then
                            if (colors[dd]) then dd = colors[dd] end
                        end
                    end; self.set(cd, dd)
                end
            end
        ...
        if (ad ~= false) then
            if (self:isType("Container")) then
                ...
                for dd, __a in
                ipairs(cd) do if (__a and __a.applyTheme) then __a:applyTheme() end end
            end
        end; return self
    end
    ...
    function _d.setTheme(ad) cb.default = ad end
    ...
    end; return { BaseElement = _c, API = _d }
end
project["plugins/xml.lua"] = function(...)
    ...
    local cb = {
        new = function(ad)
            return
            {
                ...
                addAttribute = function(bd, cd, dd) bd.attributes[cd] = dd end
            }
        end
    }
    local db = function(ad, bd)
        ...
        local __a, a_a = string.gsub(bd, "(%w+)={(.-)}", function(b_a, c_a) ad:addAttribute(b_a, c_a) end)
    end
    local _c = {
        parseText = function(ad)
            ...
            local cd = cb.new()
            table.insert(bd, cd)
            local dd, __a, a_a, b_a, c_a; local d_a, _aa = 1, 1
            while
                true do
                dd, _aa, __a, a_a, b_a, c_a = string.find(ad, "<(%/?)([%w_:]+)(.-)(%/?)>", d_a)
                if not dd then break end; local aaa = string.sub(ad, d_a, dd - 1)
                if not
                    string.find(aaa, "^%s*$") then
    ...
    local function ac(ad)
        ...
        while true do
            ...
            table.insert(bd, { start = dd, ending = __a, expression = a_a, raw = ad:sub(dd, __a) })
            cd = __a + 1
        end; return bd
    end
    local function bc(ad, bd)
        if type(ad) ~= "string" then return ad end
        if ad:sub(1, 1) == "\"" and
            ad:sub(-1) == "\"" then
            ad = ad:sub(2, -2)
        end; local cd = ac(ad)
        for dd, __a in ipairs(cd) do
            local a_a = __a.expression; local b_a = __a.start - 1
        ...
        if ad:match("^%s*<!%[CDATA%[.*%]%]>%s*$") then
            ...
            local __a = _ENV; for a_a, b_a in pairs(bd) do __a[a_a] = b_a end; return
                load("return " .. dd, nil, "bt", __a)()
        end
        if ad == "true" then
            return true
    ...
    function dc.setup(ad)
        ad.defineProperty(ad, "customXML", { default = { attributes = {}, children = {} }, type = "table" })
    end
                ...
                elseif self[cd] then
                    if (
                        ...
                        if (bd[__a]) then
                            if
                                ...
                                ab.error("XMLParser: variable '" .. __a ..
                                    "' is not a function for element '" .. self:getType() .. "' " .. cd)
                            end
                            self[cd](self, bd[__a])
                        else
                        ...
                        ab.error("XMLParser: property '" .. cd ..
                            "' not found in element '" .. self:getType() .. "'")
                    end
                else
        ...
        if (ad.children) then
            for cd, dd in pairs(ad.children) do
                if (self._properties[dd.tag]) then
                    if (
                            self._properties[dd.tag].type == "table") then
    ...
    end; return { API = _c, Container = _d, BaseElement = dc }
end
project["plugins/animation.lua"] = function(...)
    local aa = {}
    local ba = {
        linear = function(ab) return ab end,
        easeInQuad = function(ab) return ab * ab end,
        easeOutQuad = function(ab)
            return
                1 - (1 - ab) * (1 - ab)
        end,
        easeInOutQuad = function(ab)
            if ab < 0.5 then return 2 * ab * ab end; return 1 - (
                -2 * ab + 2) ^ 2 / 2
        end
    }
    local ca = {}
    ...
    function ca.new(ab, bb, cb, db, _c)
        ...
        ac.handlers = aa[bb]
        ac.easing = _c; return ac
    end; function ca:start()
        self.startTime = os.epoch("local") / 1000; if
            self.handlers.start then
    ...
    function da.registerAnimation(ab, bb)
        ...
        da[ab] = function(cb, ...)
            ...
            local _c = "linear"
            if (
                    type(db[#db]) == "string") then
                _c = table.remove(db, #db)
            end; local ac = table.remove(db, #db)
            return cb:addAnimation(ab, db, ac, _c)
        end
    end; function da.registerEasing(ab, bb) ba[ab] = bb end
    ...
    function da.new(ab)
        ...
        bb.currentSequence = 1; bb.timer = nil
        setmetatable(bb, da)
        return bb
    end
    ...
    function da:sequence()
        ...
        self.currentSequence = #
            self.sequences; self.sequenceCallbacks[self.currentSequence] = {
            start = nil,
            update = nil,
            complete =
                nil
    ...
    function da:onStart(ab)
        if
            not self.sequenceCallbacks[self.currentSequence] then
            self.sequenceCallbacks[self.currentSequence] = {}
        end
        self.sequenceCallbacks[self.currentSequence].start = ab; return self
    end
    ...
    function da:onUpdate(ab)
        ...
        self.sequenceCallbacks[self.currentSequence].update = ab; return self
    end
    ...
    function da:onComplete(ab)
        ...
        self.sequenceCallbacks[self.currentSequence].complete = ab; return self
    end
    ...
    function da:start()
        self.currentSequence = 1; self.timer = nil
        if
            (self.sequenceCallbacks[self.currentSequence]) then
            if (self.sequenceCallbacks[self.currentSequence].start) then
                self.sequenceCallbacks[self.currentSequence].start(self.element)
            end
        end
        if
            #self.sequences[self.currentSequence] > 0 then
            self.timer = os.startTimer(0.05)
            for ab, bb in
            ipairs(self.sequences[self.currentSequence]) do bb:start() end
        end; return self
    end
    ...
    function da:event(ab, bb)
        if ab == "timer" and bb == self.timer then
            ...
            for bc, cc in ipairs(self.sequences[self.currentSequence]) do
                local dc = cb - cc.startTime; local _d = dc / cc.duration; local ad = cc:update(dc)
            ...
            if db then
                ...
                if self.currentSequence < #self.sequences then
                    ...
                    if bc and bc.start then bc.start(self.element) end
                    for cc, dc in
                    ipairs(self.sequences[self.currentSequence]) do
                        dc:start()
                        table.insert(_c, dc)
                    end
                end
            end; if #_c > 0 then self.timer = os.startTimer(0.05) end
            return true
        end
    end
    ...
    da.registerAnimation("moveOffset", {
        ...
        update = function(ab, bb)
            local cb = ab.startX + (ab.args[1] - ab.startX) *
        ...
        update = function(ab, bb)
            ...
            for i = 1, ab.length do
                ...
                if bb < 0.5 then
                    ...
                    cb = cb ..
                        (math.random() > (bb - 0.5) * 2 and " " or _c)
                end
            end; ab.element.set(ab.args[1], cb)
            return bb >= 1
        end,
        complete = function(ab)
    ...
    da.registerAnimation("fadeText", {
        start = function(ab)
            ...
            for i = 1, #ab.args[2] do
                ab.chars[i] = { char = ab.args[2]:sub(i, i), visible = false }
            end
        end,
        update = function(ab, bb)
            local cb = ""
    ...
    function _b.setup(ab)
        ab.defineProperty(ab, "animation", { default = nil, type = "table" })
...
project["plugins/benchmark.lua"] = function(...)
    ...
    function bb:startProfile(_c)
        ...
        bc.startTime = os.clock() * 1000; bc.path = {}
        local cc = self; while cc do
            table.insert(bc.path, 1,
                cc.get("name") or cc.get("id"))
            cc = cc.parent
        end; return self
    end
    ...
    function bb:benchmark(_c)
        ...
        da[self].methodName = _c; da[self].isRunning = true; ab(self, _c)
        return self
    end
    ...
    function bb:logBenchmark(_c)
        ...
        if bc then
            ...
            ca.info(string.format(
                ...
                "Last time: %.2fms " .. "Total time: %.2fms", table.concat(bc.path, "."), bc.methodName,
                table.concat(bc.path, "/"), bc.calls, cc,
                bc.minTime ~= math.huge and bc.minTime or 0, bc.maxTime, bc.lastTime, bc.totalTime))
        end; return self
    end
    ...
    function bb:stopBenchmark(_c)
        ...
        end; ac.methods[_c] = nil; if
            not next(ac.methods) then
            da[self] = nil
        end; return self
    end
    ...
    function bb:getBenchmarkStats(_c)
        local ac = da[self]
        if not ac or not ac.methods[_c] then return nil end; local bc = ac.methods[_c]
        return
        {
    ...
    function cb:logContainerBenchmarks(_c, ac)
        ac = ac or 0; local bc = string.rep("  ", ac)
        local cc = 0; local dc = {}
        for ad, bd in
        pairs(self.get("children")) do
            local cd = da[bd]
            ...
            if #dc > 0 then
                for dd, __a in ipairs(dc) do
                    if __a.element:isType("Container") then
                        __a.element:logContainerBenchmarks(_c,
                            ac + 1)
                    else
                        ca.info(string.format("%s> %s: %.2fms/call [Total: %dms, Calls: %d]",
                            bc .. " ", __a.type, __a.avgTime, __a.totalTime, __a.calls))
                    end
                end
            end
        end; return self
    ...
    function db.start(_c, ac)
        ...
        bc.lastTime = 0; da[_c] = bc
    end

    function db.stop(_c)
        local ac = da[_c]
        if not ac or not ac.custom then return end; local bc =
        ...
        ca.info(string.format("Custom Benchmark '%s': " ..
            ...
            ac.totalTime / ac.calls, ac.minTime, ac.maxTime, ac.lastTime, ac.totalTime))
    end

    function db.getStats(_c)
        local ac = da[_c]
        if not ac then return nil end; return
        {
    ...
    end; function db.clear(_c) da[_c] = nil end; function db.clearAll()
        for _c, ac in pairs(da) do
            if ac.custom then da[_c] = nil end
        end
    end; return
    { BaseElement = bb, Container = cb, API = db }
end
project["plugins/state.lua"] = function(...)
    ...
    function ba.setup(da)
        ...
        da.defineProperty(da, "stateObserver", { default = {}, type = "table" })
    end
    ...
    function ba:initializeState(da, _b, ab, bb)
        ...
        if ab and
            ...
            _c =
                textutils.unserialize(ac.readAll()) or {}
            ac.close()
        end; cb[da] = { value = ab and _c[da] or _b, persist = ab }
        return self
    end; local ca = {}
    function ca:setState(da, _b)
        ...
        if bb[da].persist then
            ...
            if fs.exists(db) then
                ...
                _c =
                    textutils.unserialize(cc.readAll()) or {}
                cc.close()
            end; _c[da] = _b; local ac = fs.getDir(db)
            if not fs.exists(ac) then
                fs.makeDir(ac)
            end; local bc = fs.open(db, "w")
    ...
    function ca:getState(da)
        ...
        end; if ab[da].computed then
            return ab[da].computeFn(self)
        end; return ab[da].value
    end
    ...
    function ca:onStateChange(da, _b)
        ...
        local cb = ab.get("stateObserver")
        if not cb[da] then cb[da] = {} end; table.insert(cb[da], _b)
        return self
    end
    ...
    function ca:computed(da, _b)
        ...
        if bb[da] then
            aa.error(
                "Computed state '" .. da .. "' already exists")
            return self
        end
        bb[da] = { computeFn = _b, value = _b(self), computed = true }
        return self
    end

    function ca:bind(da, _b)
        _b = _b or da; local ab = self:getBaseFrame()
        local bb = false
        if
            self.get(da) ~= nil then
            self.set(da, ab:getState(_b))
        end
        self:onChange(da, function(cb, db)
            if bb then return end; bb = true; cb:setState(_b, db)
            bb = false
        end)
        self:onStateChange(_b, function(cb, db)
            if bb then return end; bb = true; if self.get(da) ~= nil then
                self.set(da, db)
            end; bb = false
        end)
        return self
    end; return { BaseElement = ca, BaseFrame = ba }
end
project["main.lua"] = function(...)
    ...
    local __a = {}
    __a.traceback = true; __a._events = {}
    __a._schedule = {}
    __a._eventQueue = {}
    __a._plugins = {}
    ...
    end; local a_a = nil; local b_a = nil; local c_a = {}
    local d_a = type; local _aa = {}
    local aaa = 10; local baa = 0; local caa = false
    local function daa()
        if (caa) then return end; baa = os.startTimer(0.2)
        caa = true
    end
    local function _ba(aca)
        for _ = 1, aca do
            local bca = _aa[1]
            if (bca) then bca:create() end
            table.remove(_aa, 1)
        end
    end; local function aba(aca, bca)
        if (aca == "timer") then
            if (bca == baa) then
                _ba(aaa)
                caa = false; baa = 0; if (#_aa > 0) then daa() end
                return true
            end
        end
    end
    ...
    end; function __a.getElementManager() return ad end
    ...
    function __a.getMainFrame()
        ...
        if (c_a[aca] == nil) then
            a_a = aca; __a.createFrame()
        end; return c_a[aca]
    end

    function __a.setActiveFrame(aca, bca)
        local cca = aca:getTerm()
        if (bca == nil) then bca = true end
        if (cca ~= nil) then
            c_a[tostring(cca)] =
                bca and aca or nil; aca:updateRender()
        end
    end; function __a.getActiveFrame(aca)
        if (aca == nil) then aca = term.current() end; return
            c_a[tostring(aca)]
    end
    ...
    function __a.setFocus(aca)
        ...
        if (b_a ~= nil) then b_a:dispatchEvent("focus") end
    end; function __a.getFocus() return b_a end
    ...
    function __a.schedule(aca)
        ...
        if (cca) then
            table.insert(__a._schedule, { coroutine = bca, filter = dca })
        else
    ...
    local bba = { mouse_click = true, mouse_up = true, mouse_scroll = true, mouse_drag = true }
    local cba = { key = true, key_up = true, char = true }
    local function dba(aca, ...)
        if (aca == "terminate") then
            __a.stop()
            return
        end; if aba(aca, ...) then return end; local bca = { ... }
        local function cca()
            if (bba[aca]) then
                if
                    c_a[a_a] then
        ...
        for bda, cda in pairs(__a._eventQueue) do
            ...
            end; if coroutine.status(cda.coroutine) == "dead" then
                table.remove(__a._eventQueue, bda)
            end
        end; local dca = { coroutine = coroutine.create(cca), filter = aca }
        local _da, ada = coroutine.resume(dca.coroutine, aca, ...)
        if (not _da) then
            bd.header = "Basalt Event Error"
            bd.error(ada)
        end; if (ada ~= nil) then dca.filter = ada end
        ...
        end; if __a._events[aca] then
            for bda, cda in ipairs(__a._events[aca]) do cda(...) end
        end
    end; local function _ca()
        for aca, bca in pairs(c_a) do
            bca:render()
            bca:postRender()
        end
    end; function __a.render() _ca() end

    function __a.update(...)
        local aca = function(...)
            __a.isRunning = true; dba(...)
            _ca()
        end
        ...
        if not (bca) then
            ...
            bd.error(cca)
        end; __a.isRunning = false
    end; function __a.stop()
        __a.isRunning = false; term.clear()
    ...
    end; function __a.getElementClass(aca) return ad.getElement(aca) end; function __a.getAPI(aca)
        return
            ad.getAPI(aca)
    end; return __a
end
project["render.lua"] = function(...)
    ...
    local ba = {}
    ba.__index = ba; local ca = string.sub
    function ba.new(da)
    ...
    end; function ba:addDirtyRect(da, _b, ab, bb)
        table.insert(self.buffer.dirtyRects, { x = da, y = _b, width = ab, height = bb })
        return self
    end

    function ba:blit(da, _b, ab, bb, cb)
        if
            _b < 1 or _b > self.height then
            return self
        end; if (#ab ~= #bb or
                #ab ~= #cb) then
        ...
        self.buffer.bg[_b] = ca(
            self.buffer.bg[_b]:sub(1, da - 1) .. cb .. self.buffer.bg[_b]:sub(da + #cb), 1, self.width)
        self:addDirtyRect(da, _b, #ab, 1)
        return self
    end

    function ba:multiBlit(da, _b, ab, bb, cb, db, _c)
        if _b < 1 or _b > self.height then return self end; if (
                #cb ~= #db or #cb ~= #_c) then
            error("Text, fg, and bg must be the same length")
        end; cb = cb:rep(ab)
        db = db:rep(ab)
        ...
        for dy = 0, bb - 1 do
            ...
            if ac >= 1 and ac <= self.height then
                ...
                self.buffer.bg[ac] = ca(
                    self.buffer.bg[ac]:sub(1, da - 1) .. _c .. self.buffer.bg[ac]:sub(da + #_c), 1, self.width)
            end
        end; self:addDirtyRect(da, _b, ab, bb)
        return self
    end

    function ba:textFg(da, _b, ab, bb)
        if _b < 1 or _b > self.height then return self end
        bb = _a[bb] or "0"
        bb = bb:rep(#ab)
        self.buffer.text[_b] = ca(self.buffer.text[_b]:sub(1,
                da - 1) .. ab ..
    ...
    function ba:textBg(da, _b, ab, bb)
        ...
        self.buffer.bg[_b] = ca(
            ...
            bb:rep(#ab) .. self.buffer.bg[_b]:sub(da + #ab), 1, self.width)
        self:addDirtyRect(da, _b, #ab, 1)
        return self
    end
    ...
    function ba:fg(da, _b, ab)
        ...
        self.buffer.fg[_b] = ca(self.buffer.fg[_b]:sub(1,
                da - 1) .. ab ..
            self.buffer.fg[_b]:sub(da + #ab), 1, self.width)
        self:addDirtyRect(da, _b, #ab, 1)
        return self
    end
    ...
    function ba:bg(da, _b, ab)
        ...
        self.buffer.bg[_b] = ca(self.buffer.bg[_b]:sub(1,
                da - 1) .. ab ..
            self.buffer.bg[_b]:sub(da + #ab), 1, self.width)
        self:addDirtyRect(da, _b, #ab, 1)
        return self
    end
    ...
    function ba:render()
        ...
        for _b, ab in ipairs(self.buffer.dirtyRects) do
            local bb = false; for cb, db in ipairs(da) do
                if
                    self:rectOverlaps(ab, db) then
    ...
    function ba:mergeRects(da, _b)
        ...
        da.x = ab; da.y = bb; da.width = cb - ab; da.height = db - bb; return self
    end
    ...
    function ba:setCursor(da, _b, ab, bb)
        ...
        self.xCursor = da; self.yCursor = _b; self.blink = ab; self.cursorColor = bb
        return self
    end
    ...
    function ba:clearArea(da, _b, ab, bb, cb)
        ...
        for dy = 0, bb - 1 do
            local _c = _b + dy; if
                _c >= 1 and _c <= self.height then
                local ac = string.rep(" ", ab)
                local bc = string.rep(db, ab)
                self:blit(da, _c, ac, "0", db)
            end
        end; return self
    end; function ba:getSize() return self.width, self.height end
...
project["libraries/expect.lua"] = function(...)
    ...
    local function d(_a, aa, ba)
        ...
        if ba == "element" then
            if ca == "table" and
                aa.get("type") ~= nil then
                return true
            end
        end
        if ba == "color" then
            if ca == "number" then return true end; if
                ca == "string" and colors[aa] then
                return true
            end
        end; if ca ~= ba then
...
project["libraries/utils.lua"] = function(...)
    local d, _a = math.floor, string.len; local aa = {}
    ...
    function aa.deepCopy(ba)
        if type(ba) ~= "table" then return ba end; local ca = {}
    ...
    function aa.copy(ba)
        local ca = {}
        for da, _b in pairs(ba) do ca[da] = _b end; return ca
    end; function aa.reverse(ba)
        local ca = {}
        for i = #ba, 1, -1 do table.insert(ca, ba[i]) end
        return ca
    end

    function aa.uuid()
        return
            string.format('%04x%04x-%04x-%04x-%04x-%04x%04x%04x', math.random(0, 0xffff), math.random(0, 0xffff),
                math.random(0, 0xffff),
                math.random(0, 0x0fff) + 0x4000, math.random(0, 0x3fff) + 0x8000, math.random(0, 0xffff),
    ...
    end; function aa.removeTags(ba) return ba:gsub("{[^}]+}", "") end

    function aa.wrapText(ba, ca)
        if
            ba == nil then
            return {}
        end; ba = aa.removeTags(ba)
        local da = {}
        ...
        for ab, bb in ipairs(_b) do
            if #bb == 0 then
                ...
                for db, _c in ipairs(cb) do
                    ...
                    end; if #bc > 0 then table.insert(da, bc) end
                end; if ab < #_b then table.insert(da, "") end
            end
        end; return da
    end; return aa
end
project["elements/Scrollbar.lua"] = function(...)
    ...
    ca.defineProperty(ca, "handleSize", { default = 2, type = "number", canTriggerRender = true })
    ...
    function ca.new()
        ...
        ab.class = ca; ab.set("width", 1)
        ab.set("height", 10)
        return ab
    end; function ca:init(ab, bb)
        aa.init(self, ab, bb)
    ...
    function ca:updateAttachedElement()
        ...
        if not ab then return end; local bb = self.get("value")
        local cb = self.get("minValue")
        local db = self.get("maxValue")
        if type(cb) == "function" then cb = cb() end; if type(db) == "function" then
            db = db()
        end; local _c = cb + (bb / 100) * (db - cb)
        ab.set(self.get("attachedProperty"), math.floor(
            _c + 0.5))
        return self
    end; local function da(ab)
        return
            ab.get("orientation") == "vertical" and ab.get("height") or ab.get("width")
    end
    local function _b(ab, bb, cb)
        local db, _c = ab:getRelativePosition(bb, cb)
        return
            ab.get("orientation") == "vertical" and _c or db
    end
...
project["elements/Display.lua"] = function(...)
    ...
    bb.__index = bb; function bb.new()
        local cb = setmetatable({}, bb):__init()
        cb.class = bb; cb.set("width", 25)
        cb.set("height", 8)
        cb.set("z", 5)
        return cb
    end
    ...
    function bb:init(cb, db)
        ...
        local _c = self._window.reposition; local ac = self._window.blit
        ...
        self:observe("width", function(cc, dc)
            local _d = cc._window; if _d then
    ...
    end; function bb:getWindow() return self._window end
    ...
    function bb:write(cb, db, _c, ac, bc)
        ...
        if cc then
            if ac then
                cc.setTextColor(ac)
            end; if bc then cc.setBackgroundColor(bc) end
            cc.setCursorPos(cb, db)
            cc.write(_c)
        end; self:updateRender()
        return self
    end

    function bb:render()
        ca.render(self)
        local cb = self._window; local db, _c = cb.getSize()
        if cb then
            for y = 1, _c do
...
project["elements/Dropdown.lua"] = function(...)
    ...
    function ca:mouse_click(da, _b, ab)
        ...
        if cb == 1 then
            ...
            return true
        elseif
            self.get("isOpen") and cb > 1 and self.get("selectable") then
            ...
            if db <= #_c then
                ...
                end; ac.selected = not ac.selected
    ...
    function ca:render()
        ...
        self:blit(1, 1, da .. string.rep(" ", self.get("width") - #
                da - 1) .. (
                self.get("isOpen") and "\31" or "\17"), string.rep(ba[self.get("foreground")], self.get("width")),
            string.rep(ba[self.get("background")], self.get("width")))
        if self.get("isOpen") then
            local ab = self.get("items")
            local bb = self.get("height") - 1; local cb = self.get("offset")
            ...
            for i = 1, bb do
                ...
                if ac then
                    ...
                    if
                        ...
                        local bc = ac.text; local cc = ac.selected; bc = bc:sub(1, db)
                        local dc = cc and (ac.selectedBackground or
                            self.get("selectedBackground")) or (ac.background or
                            self.get("background"))
                        local _d =
                            cc and (ac.selectedForeground or self.get("selectedForeground")) or
...
project["elements/LineChart.lua"] = function(...)
    ...
    ab.__index = ab; function ab.new()
        local cb = setmetatable({}, ab):__init()
        cb.class = ab; return cb
    end
    ...
    local function bb(cb, db, _c, ac, bc, cc, dc, _d)
        ...
        for i = 0, cd do
            ...
            if
                ...
                __a >= 1 and __a <= cb.get("width") and a_a >= 1 and a_a <= cb.get("height") then
...
project["elements/Switch.lua"] = function(...)
    ...
    aa.__index = aa
    aa.defineProperty(aa, "checked", { default = false, type = "boolean", canTriggerRender = true })
    aa.defineEvent(aa, "mouse_click")
    aa.defineEvent(aa, "mouse_up")
    function aa.new()
        local ba = setmetatable({}, aa):__init()
        ba.class = aa; ba.set("width", 2)
        ba.set("height", 1)
        ba.set("z", 5)
        return ba
    end; function aa:init(ba, ca)
        _a.init(self, ba, ca)
        self.set("type", "Switch")
    end; function aa:render()
        _a.render(self)
    end; return aa
end
    ...
    function ca:setItems(da)
        ...
        for bb, cb in ipairs(da) do
            if cb.separator then
                table.insert(_b, { text = cb.text or "|", selectable = false })
                ab = ab + 1
            else
                local db = " " .. cb.text .. " "
                cb.text = db
        ...
        for _b, ab in ipairs(self.get("items")) do
            ...
            local cb =
                ab.selectable == false and self.get("separatorColor") or (bb and (ab.selectedForeground or
                    ...
                    self.get("foreground")))
            local db =
                bb and (ab.selectedBackground or self.get("selectedBackground")) or
                ...
                if _c.selectable ~= false then
                    ...
                    if
                        ...
                        for ac, bc in ipairs(self.get("items")) do
                            if type(bc) == "table" then bc.selected = false end
                        end
                    end; _c.selected = not _c.selected
                    if _c.callback then _c.callback(self) end; self:fireEvent("select", db, _c)
                end; return true
            end; cb = cb + #_c.text
        end; return false
    end; return ca
...
project["elements/Slider.lua"] = function(...)
    ...
    aa.defineProperty(aa, "sliderColor", { default = colors.blue, type = "color", canTriggerRender = true })
    aa.defineEvent(aa, "mouse_click")
    ...
    function aa:getValue()
        ...
        local da =
            self.get("horizontal") and self.get("width") or self.get("height")
    ...
    end; aa.mouse_drag = aa.mouse_click
    function aa:mouse_scroll(ba, ca, da)
        if self:isInBounds(ca, da) then
            local _b = self.get("step")
            local ab = self.get("horizontal") and self.get("width") or
                self.get("height")
...
project["elements/Frame.lua"] = function(...)
    ...
    function ca.new()
        local da = setmetatable({}, ca):__init()
        da.class = ca; da.set("width", 12)
        da.set("height", 6)
        da.set("background", colors.gray)
        da.set("z", 10)
        return da
    end; function ca:init(da, _b)
        ba.init(self, da, _b)
        self.set("type", "Frame")
        return self
    end
        ...
        if
            ...
            if self.get("draggable") then
                ...
                for _c, ac in ipairs(db) do
                    local bc = ac.width or 1; local cc = ac.height or 1; if
                        type(bc) == "string" and bc == "width" then
                        bc = self.get("width")
                    elseif
                        type(bc) == "function" then
                        bc = bc(self)
                    end
                    if type(cc) ==
                        "string" and cc == "height" then
                        cc = self.get("height")
                    elseif
                        type(cc) == "function" then
                        cc = cc(self)
                    end; local dc = ac.y or 1
                    if
            ...
            end; return ba.mouse_click(self, da, _b, ab)
        end; return false
    end

    function ca:mouse_up(da, _b, ab)
        self.dragging = false; self.dragStartX = nil; self.dragStartY = nil; return
            ba.mouse_up(self, da, _b, ab)
    end
    ...
    function ca:mouse_drag(da, _b, ab)
        ...
        if not self.dragging then return ba.mouse_drag(self, da, _b, ab) end; return false
    end; return ca
end
project["elements/Flexbox.lua"] = function(...)
    ...
    ab.defineProperty(ab, "flexAlignItems", {
        ...
        setter = function(bc, cc)
            if
                not cc:match("^flex%-") and cc ~= "stretch" then
                cc = "flex-" .. cc
            end; return cc
        end
    })
    ...
    local bb = {
        getHeight = function(bc) return 0 end,
        getWidth = function(bc) return 0 end,
        getZ = function(bc) return 1 end,
        getPosition = function(bc) return 0, 0 end,
        getSize = function(bc) return 0, 0 end,
        isType = function(bc)
            return
                false
        end,
        getType = function(bc) return "lineBreak" end,
        getName = function(bc) return "lineBreak" end,
        setPosition = function(bc)
        end,
        setParent = function(bc) end,
        setSize = function(bc) end,
        getFlexGrow = function(bc) return 0 end,
        getFlexShrink = function(bc) return 0 end,
        getFlexBasis = function(bc) return 0 end,
        init = function(bc) end,
    ...
    local function cb(bc, cc, dc, _d)
        ...
        for __a, a_a in pairs(bc.get("children")) do
            if a_a.get("visible") then
                table.insert(bd, a_a)
                if a_a ~= bb then cd = cd + 1 end
            end
        end; if cd == 0 then return ad end
        if not _d then
            ad[1] = { offset = 1 }
            ...
            for c_a, d_a in ipairs(a_a) do
                if #d_a == 0 then
                    ...
                    for caa, daa in ipairs(d_a) do
                        ...
                        local bba = false
                        if cc == "row" then
                            local _ca, aca = pcall(function() return daa.get("intrinsicWidth") end)
                            if _ca and aca then
                                _ba = aca; bba = true
                            end
                        else
                        ...
                        end; local cba = bba and _ba or aba; local dba = cba; if #aaa > 0 then dba = dba + dc end
                        if
                            baa + dba <= __a or #aaa == 0 then
                    ...
                    end; if #aaa > 0 then table.insert(_aa, aaa) end; for caa, daa in ipairs(_aa) do
                        ad[#ad + 1] = { offset = 1 }
                        for _ba, aba in ipairs(daa) do table.insert(ad[#ad], aba) end
                    end
                end
            end
        end; local dd = {}
        for __a, a_a in ipairs(ad) do if #a_a > 0 then table.insert(dd, a_a) end end; return dd
    end
    local function db(bc, cc, dc, _d)
        local ad = {}
        for dca, _da in ipairs(cc) do if _da ~= bb then table.insert(ad, _da) end end; if #ad == 0 then return end; local bd =
            bc.get("width")
        local cd = bc.get("height")
        local dd = bc.get("flexAlignItems")
        local __a = bc.get("flexCrossPadding")
        local a_a = bc.get("flexWrap")
        if bd <= 0 then return end; local b_a = cd - (__a * 2)
        if b_a <
            1 then
            b_a = cd; __a = 0
        end; local c_a = math.max; local d_a = math.min
        local _aa = math.floor; local aaa = math.ceil; local baa = 0; local caa = 0; local daa = {}
        local _ba = {}
        local aba = {}
        for dca, _da in ipairs(ad) do
            local ada =
                _da.get("flexGrow") or 0
            local bda = _da.get("flexShrink") or 0; local cda = _da.get("width")
            _ba[_da] = ada; aba[_da] = bda; daa[_da] = cda; if
                ada > 0 then
                caa = caa + ada
            else
                baa = baa + cda
            end
        end; local bba = #ad; local cba = (bba > 1) and ((bba - 1) * dc) or 0; local dba =
            bd - baa - cba
        ...
        elseif dba < 0 then
            ...
            if
                dca > 0 and #_da > 0 then
                local ada = -dba; for bda, cda in ipairs(_da) do
                    local dda = cda.get("width")
                    local __b = aba[cda]
                    local a_b = __b / dca; local b_b = aaa(ada * a_b)
        ...
        for dca, _da in ipairs(ad) do
            ...
            local ada = _da.get("y") + _da.get("height") - 1; if
                ...
                _ca + _da.get("width") + dc
        end; local aca = ad[#ad]
        local bca = 0; if aca then
            bca = aca.get("x") + aca.get("width") - 1
        end; local cca = bd - bca
        if cca > 0 then
            if _d == "flex-end" then
                for dca, _da in ipairs(ad) do
                    _da.set("x",
    ...
    local function _c(bc, cc, dc, _d)
        ...
        local __a = bc.get("flexCrossPadding")
        local a_a = bc.get("flexWrap")
        if cd <= 0 then return end; local b_a = bd - (__a * 2)
        if b_a <
            1 then
            b_a = bd; __a = 0
        end; local c_a = math.max; local d_a = math.min
        ...
        for dca, _da in ipairs(ad) do
            local ada =
                _da.get("flexGrow") or 0
            local bda = _da.get("flexShrink") or 0; local cda = _da.get("height")
            _ba[_da] = ada; aba[_da] = bda; daa[_da] = cda; if
                ada > 0 then
                caa = caa + ada
            else
        ...
        for dca, _da in ipairs(ad) do
            ...
            local ada = _da.get("x") + _da.get("width") - 1; if
                ...
                _ca + _da.get("height") + dc
        end; local aca = ad[#ad]
        local bca = 0; if aca then
            bca = aca.get("y") + aca.get("height") - 1
        end; local cca = cd - bca
        if cca > 0 then
            if _d == "flex-end" then
                for dca, _da in ipairs(ad) do
                    _da.set("y",
        ...
        if
            ...
            for b_a, c_a in pairs(bc.get("children")) do
                if
                    ...
                    if cc == "row" then
                        local d_a, _aa = pcall(function() return c_a.get("intrinsicWidth") end)
                        if d_a and _aa then c_a.set("width", _aa) end
                    else
                        local d_a, _aa = pcall(function()
                            return
                                c_a.get("intrinsicHeight")
                        end)
                        if d_a and _aa then c_a.set("height", _aa) end
                    end
                end
            end
        end; local __a = cb(bc, cc, dc, ad)
        if #__a == 0 then return end
        ...
        elseif cc == "column" and ad then
            ...
            for c_a, d_a in ipairs(__a) do
                if #d_a > 0 then
                    ...
                    local _aa = 0; for aaa, baa in ipairs(d_a) do
                        if baa ~= bb then _aa = math.max(_aa, baa.get("width")) end
                    end; if c_a < #__a then
                        b_a = b_a + _aa + dc
                    else
                        b_a =
            ...
            for b_a, c_a in ipairs(__a) do a_a(bc, c_a, dc, _d) end
        end; bc:sortChildren()
    ...
    function ab.new()
        ...
        bc.class = ab; bc.set("width", 12)
        bc.set("height", 6)
        bc.set("background", colors.blue)
        bc.set("z", 10)
        bc._lastLayoutWidth = 0; bc._lastLayoutHeight = 0
        ...
        bc:observe("flexCrossPadding", function() bc.set("flexUpdateLayout", true) end)
        return bc
    end; function ab:init(bc, cc)
        _b.init(self, bc, cc)
    ...
    function ab:addChild(bc)
        ...
        end; self.set("flexUpdateLayout", true)
        return self
    end
    ...
    function ab:removeChild(bc)
        ...
        if (bc ~= bb) then
            bc.setFlexGrow = nil; bc.setFlexShrink =
...
project["elements/Timer.lua"] = function(...)
    ...
    aa.defineProperty(aa, "amount", { default = -1, type = "number" })
    aa.defineEvent(aa, "timer")
    function aa.new()
        local ba = setmetatable({}, aa):__init()
        ba.class = aa; return ba
    end; function aa:init(ba, ca)
        _a.init(self, ba, ca)
        self.set("type", "Timer")
    end
    ...
    function aa:start()
        if
            not self.running then
            self.running = true; local ba = self.get("interval")
...
project["elements/VisualElement.lua"] = function(...)
    ...
    _b.defineProperty(_b, "z", {
        ...
        setter = function(cb, db)
            if cb.parent then cb.parent:sortChildren() end; return db
        end
    })
    ...
    _b.defineProperty(_b, "visible", {
        ...
        setter = function(cb, db)
            ...
            end; if (db == false) then cb.set("clicked", false) end; return db
        end
    })
    ...
    local ab, bb = math.max, math.min; function _b.new()
        local cb = setmetatable({}, _b):__init()
        cb.class = _b; return cb
    end
    ...
    function _b:multiBlit(cb, db, _c, ac, bc, cc, dc)
        ...
        cb = cb + _d - 1
        db = db + ad - 1; self.parent:multiBlit(cb, db, _c, ac, bc, cc, dc)
    end

    function _b:textFg(cb, db, _c, ac)
        local bc, cc = self:calculatePosition()
        cb = cb + bc - 1
        db = db + cc - 1; self.parent:textFg(cb, db, _c, ac)
    end

    function _b:textBg(cb, db, _c, ac)
        local bc, cc = self:calculatePosition()
        cb = cb + bc - 1
        db = db + cc - 1; self.parent:textBg(cb, db, _c, ac)
    end

    function _b:drawText(cb, db, _c)
        local ac, bc = self:calculatePosition()
        cb = cb + ac - 1
        db = db + bc - 1; self.parent:drawText(cb, db, _c)
    end

    function _b:drawFg(cb, db, _c)
        local ac, bc = self:calculatePosition()
        cb = cb + ac - 1
        db = db + bc - 1; self.parent:drawFg(cb, db, _c)
    end

    function _b:drawBg(cb, db, _c)
        local ac, bc = self:calculatePosition()
        cb = cb + ac - 1
        db = db + bc - 1; self.parent:drawBg(cb, db, _c)
    end

    function _b:blit(cb, db, _c, ac, bc)
        local cc, dc = self:calculatePosition()
        cb = cb + cc - 1
        db = db + dc - 1; self.parent:blit(cb, db, _c, ac, bc)
    end
    ...
    function _b:isInBounds(cb, db)
        ...
        if (self.get("ignoreOffset")) then
            ...
            cb >= _c and cb <=
            _c + bc - 1 and db >= ac and db <= ac + cc - 1
    end
    ...
    function _b:mouse_move(cb, db, _c)
        if (db == nil) or (_c == nil) then return end
    ...
    end; function _b:focus() self:fireEvent("focus") end; function _b:blur()
        self:fireEvent("blur")
    ...
    function _b:key(cb, db)
        if
            (self.get("focused")) then
            self:fireEvent("key", cb, db)
        end
    end; function _b:key_up(cb)
        if (self.get("focused")) then self:fireEvent("key_up", cb) end
    end; function _b:char(cb)
    ...
    function _b:getAbsolutePosition(cb, db)
        ...
        if (cb ~= nil) then
            _c = _c + cb - 1
        end; if (db ~= nil) then ac = ac + db - 1 end; local bc = self.parent
        while bc do
            local cc, dc = bc.get("x"), bc.get("y")
            _c = _c + cc - 1; ac = ac + dc - 1; bc = bc.parent
        end; return _c, ac
    end
    ...
    function _b:getRelativePosition(cb, db)
        ...
        end; local _c, ac = 1, 1; if self.parent then
    ...
    function _b:setCursor(cb, db, _c, ac)
        if self.parent then
            ...
            if
                ...
                return self.parent:setCursor(
                    cb + bc - 1, db + cc - 1, false)
            end
    ...
    function _b:prioritize()
        if (self.parent) then
            local cb = self.parent; cb:removeChild(self)
            cb:addChild(self)
            self:updateRender()
        end; return self
    end
    ...
    end; function _b:postRender() end; return _b
end
project["elements/Graph.lua"] = function(...)
    ...
    function ca:getSeries(da)
        local _b = self.get("series")
        for ab, bb in ipairs(_b) do
            if bb.name == da then return bb end
        end; return nil
    end
    ...
    function ca:addPoint(da, _b)
        ...
        for bb, cb in ipairs(ab) do
            if cb.name == da then
                ...
                while #cb.data > cb.pointCount do table.remove(cb.data, 1) end; break
            end
        end; self:updateRender()
        return self
    end
    ...
    function ca:clear(da)
        ...
        if da then
            ...
            for ab, bb in ipairs(_b) do bb.data = {} end
        end; return self
    end
...
project["elements/BigFont.lua"] = function(...)
        ...
        for f = #bb + 1, dc do
            ...
            for char = 0, 255 do
                ...
                for i = 1, #b_a do
                    ...
                    end; table.insert(__a, table.concat(d_a))
    ...
    local function _c(dc, _d, ad, bd, cd)
        ...
        if (bb[dc] == nil) then db(3, false) end; local a_a = bb[dc] or
            error("Wrong font size selected", 3)
        if _d == "" then
            return { { "" }, { "" }, { "" } }
        end; local b_a = {}
        for daa in _d:gmatch('.') do table.insert(b_a, daa) end; local c_a = {}
        local d_a = #a_a[b_a[1]][1]
        for nLine = 1, d_a do
            local daa = {}
            for i = 1, #b_a do
                daa[i] =
                    a_a[b_a[i]] and a_a[b_a[i]][1][nLine] or ""
            end; c_a[nLine] = table.concat(daa)
        end; local _aa = {}
        local aaa = {}
        local baa = { ["0"] = dd, ["1"] = __a }
        local caa = { ["0"] = __a, ["1"] = dd }
        for nLine = 1, d_a
        do
            local daa = {}
            local _ba = {}
            for i = 1, #b_a do
                ...
                _ba[i] = aba:gsub("[01]",
                    cd and { ["0"] = bd:sub(i, i), ["1"] = ad:sub(i, i) } or caa)
            end; _aa[nLine] = table.concat(daa)
            aaa[nLine] = table.concat(_ba)
        end; return { c_a, _aa, aaa }
    end; local ac = require("elementManager")
...
project["elements/BaseFrame.lua"] = function(...)
    ...
    local function bb(cb)
        local db, _c = pcall(function() return peripheral.getType(cb) end)
        if db then return true end; return false
    end
    ab.defineProperty(ab, "term", {
        default = nil,
        type = "table",
        setter = function(cb, db)
            cb._peripheralName = nil; if
                cb.basalt.getActiveFrame(cb._values.term) == cb then
                cb.basalt.setActiveFrame(cb, false)
            end; if
                db == nil or db.setCursorPos == nil then
                return db
            end; if (bb(db)) then
                cb._peripheralName = peripheral.getName(db)
            end; cb._values.term = db
            if
                cb.basalt.getActiveFrame(db) == nil then
                cb.basalt.setActiveFrame(cb)
            end; cb._render = _b.new(db)
            cb._renderUpdate = true; local _c, ac = db.getSize()
            cb.set("width", _c)
            cb.set("height", ac)
            return db
        end
    ...
    end; function ab:textFg(cb, db, _c, ac)
        if cb < 1 then
            _c = string.sub(_c, 1 - cb)
            cb = 1
        end
        self._render:textFg(cb, db, _c, ac)
    end; function ab:textBg(cb, db, _c, ac)
        if cb < 1 then
            _c = string.sub(_c, 1 -
                cb)
            cb = 1
    ...
    end; function ab:drawText(cb, db, _c)
        if cb < 1 then
            _c = string.sub(_c,
                1 - cb)
            cb = 1
    ...
    function ab:drawFg(cb, db, _c)
        if cb < 1 then
            _c = string.sub(_c, 1 - cb)
            cb = 1
        end; self._render:fg(cb, db, _c)
    end; function ab:drawBg(cb, db, _c)
        if cb < 1 then
            _c = string.sub(_c, 1 - cb)
            cb = 1
        end
    ...
    function ab:blit(cb, db, _c, ac, bc)
        if cb < 1 then
            ...
            cb = 1
        end; self._render:blit(cb, db, _c, ac, bc)
    end; function ab:setCursor(cb, db, _c, ac)
    ...
    end; function ab:mouse_click(cb, db, _c)
    ...
    function ab:term_resize()
        ...
        if (cb ==
                self.get("width") and db == self.get("height")) then
            return
        end; self.set("width", cb)
        self.set("height", db)
    ...
    function ab:dispatchEvent(cb, ...)
        ...
        if db == nil then return end; if (bb(db)) then
            if
                cb == "mouse_click" then
                return
            end
...
project["elements/Checkbox.lua"] = function(...)
    ...
    d.defineProperty(d, "text",
        {
            ...
            setter = function(_a, aa)
                ...
                if (_a.get("autoSize")) then _a.set("width", ca) end; return aa
            end
        })
    ...
    d.defineProperty(d, "autoSize", { default = true, type = "boolean" })
...
project["elements/Container.lua"] = function(...)
    ...
    db.defineProperty(db, "focusedChild",
        {
            ...
            setter = function(bc, cc, dc)
                ...
                if _d then
                    if _d:isType("Container") then _d.set("focusedChild", nil, true) end; _d.set("focused", false, true)
                end
    ...
    function db:isChildVisible(bc)
        if not bc:isType("VisualElement") then return false end; if (bc.get("visible") == false) then return false end; if (bc._destroyed) then
            return
                false
        end
        ...
        local a_a; local b_a; if
            (bc.get("ignoreOffset")) then
            a_a = bd; b_a = cd
        else
            a_a = bd - _d; b_a = cd - ad
        end; return

            (a_a + dd > 0) and (a_a <= cc) and (b_a + __a > 0) and (b_a <= dc)
    end

    function db:addChild(bc)
        if bc == self then error("Cannot add container to itself") end; if (bc ~= nil) then
            table.insert(self._values.children, bc)
            bc.parent = self; bc:postInit()
            self.set("childrenSorted", false)
    ...
    function db:registerChildrenEvents(bc)
        if (bc._registeredEvents == nil) then return end
        for cc in
        pairs(bc._registeredEvents) do self:registerChildEvent(bc, cc) end; return self
    end

    function db:registerChildEvent(bc, cc)
        if not self._values.childrenEvents[cc] then
            self._values.childrenEvents[cc] = {}
            self._values.eventListenerCount[cc] = 0; if self.parent then
                self.parent:registerChildEvent(self, cc)
            end
        end; for dc, _d in ipairs(self._values.childrenEvents[cc]) do
            if _d == bc then return self end
        end
        self.set("childrenEventsSorted", false)
        table.insert(self._values.childrenEvents[cc], bc)
        self._values.eventListenerCount[cc] =
            self._values.eventListenerCount[cc] + 1; return self
    end
        ...
        if self._values.childrenEvents[cc] then
            ...
            ipairs(self._values.childrenEvents[cc]) do
                if _d.get("id") == bc.get("id") then
                    ...
                    end; self.set("childrenEventsSorted", false)
                    self:updateRender()
                    break
                end
            end
    ...
    function db:removeChild(bc)
        if bc == nil then return self end
        ...
        end; self:removeChildrenEvents(bc)
        self:updateRender()
        self.set("childrenSorted", false)
        return self
    end
    ...
    function db:getChild(bc)
        if type(bc) == "string" then
            local cc = cb(bc, "/")
            for dc, _d in
            pairs(self._values.children) do
    ...
    function db:callChildrenEvent(bc, cc, ...)
        local dc = bc and self.get("visibleChildrenEvents") or
        ...
        if
            ...
            for i = #_d, 1, -1 do
                local ad = _d[i]
                if (ad:dispatchEvent(cc, ...)) then return true, ad end
            end
        end
        if (dc["*"]) then
            local _d = dc["*"]
    ...
    function db:handleEvent(bc, ...)
        ...
        local cc = ac(self, bc, ...)
        return
            self:callChildrenEvent(false, bc, table.unpack(cc))
    end
    ...
    function db:mouse_scroll(bc, cc, dc)
        ...
        if (ad) then return true end
        if (ab.mouse_scroll(self, bc, cc, dc)) then return true end; return false
    end; function db:key(bc)
        if self.get("focusedChild") then
            return
                self.get("focusedChild"):dispatchEvent("key", bc)
        end
        return true
    end
    ...
    end; function db:key_up(bc)
        if self.get("focusedChild") then
            return
                self.get("focusedChild"):dispatchEvent("key_up", bc)
        end; return true
    end

    function db:multiBlit(bc, cc, dc, _d, ad, bd, cd)
        local dd, __a = self.get("width"), self.get("height")
        dc = bc < 1 and math.min(dc + bc - 1, dd) or
            math.min(dc, math.max(0, dd - bc + 1))
        _d = cc < 1 and math.min(
                _d + cc - 1, __a) or
            math.min(_d, math.max(0, __a - cc + 1))
        if dc <= 0 or
            _d <= 0 then
            return self
        end
    ...
    function db:textFg(bc, cc, dc, _d)
        ...
        if
            cc < 1 or cc > bd then
            return self
        end; local cd = bc < 1 and (2 - bc) or 1
        local dd = math.min(#dc -
            cd + 1, ad - math.max(1, bc) + 1)
        if dd <= 0 then return self end
    ...
    function db:drawText(bc, cc, dc)
        ...
        if cc < 1 or
            cc > ad then
            return self
        end; local bd = bc < 1 and (2 - bc) or 1
        local cd = math.min(
            #dc - bd + 1, _d - math.max(1, bc) + 1)
        if cd <= 0 then return self end
    ...
    function db:blit(bc, cc, dc, _d, ad)
        ...
        if
            cc < 1 or cc > cd then
            return self
        end; local dd = bc < 1 and (2 - bc) or 1
        ...
        if __a <= 0 then return self end; local c_a = dc:sub(dd, dd + __a - 1)
        local d_a = _d:sub(dd,
            dd + a_a - 1)
    ...
    function db:render()
        ...
        if
            not self.get("childrenEventsSorted") then
            for bc in pairs(self._values.childrenEvents) do
                self:sortChildrenEvents(bc)
            end
        end; for bc, cc in ipairs(self.get("visibleChildren")) do
            if cc == self then
...
project["elements/List.lua"] = function(...)
    ...
    d.defineProperty(d, "selectedForeground", { default = colors.white, type = "color" })
    d.defineEvent(d, "mouse_click")
    ...
    function d:getSelectedItems()
        ...
        for aa, ba in ipairs(self.get("items")) do
            if
                type(ba) == "table" and ba.selected then
                local ca = ba; ca.index = aa; table.insert(_a, ca)
            end
        end; return _a
    end
        ...
        if
            ...
            if _b <= #ab then
                ...
                if type(bb) == "string" then
                    ...
                    not self.get("multiSelection") then
                    for cb, db in ipairs(ab) do if type(db) == "table" then db.selected = false end end
                end
    ...
    function d:mouse_scroll(_a, aa, ba)
        if self:isInBounds(aa, ba) then
            ...
            local da = math.max(0, #
                self.get("items") - self.get("height"))
    ...
    function d:selectItem(_a)
        ...
        if
            not self.get("multiSelection") then
            for ca, da in ipairs(aa) do if type(da) == "table" then da.selected = false end end
        end; local ba = aa[_a]
        if
            type(ba) == "string" then
            ba = { text = ba }
        ...
        for i = 1, aa do
            ...
            if _b then
                ...
                if _b.separator then
                    ...
                    local cb =
                        bb and (_b.selectedBackground or self.get("selectedBackground")) or
                        (_b.background or self.get("background"))
                    local db =
                        bb and (_b.selectedForeground or self.get("selectedForeground")) or
...
project["elements/Program.lua"] = function(...)
    ...
    ab.defineProperty(ab, "doneCallback", { default = nil, type = "function" })
    ab.defineEvent(ab, "*")
    local bb = {}
    bb.__index = bb
    ...
    function bb.new(_c, ac, bc)
        ...
        cc.args = {}
        cc.addEnvironment =
            bc == nil and true or bc; cc.program = _c; return cc
    end; function bb:setArgs(...) self.args = { ... } end

    local function db(_c)
        local ac = { shell = shell, multishell = multishell }
    ...
    function bb:run(_c, ac, bc)
        ...
        if (cc ~= nil) then
            if (fs.exists(cc)) then
                ...
                local _d = dc.readAll()
                dc.close()
                local ad = setmetatable(db(fs.getDir(_c)), { __index = _ENV })
                ad.term = self.window; ad.term.current = term.current
                ad.term.redirect = term.redirect; ad.term.native = function() return self.window end
                if
                    (self.addEnvironment) then
                ...
                if not cd then
                    ...
                    if __a then __a(self.program, cd, dd) end
                    ...
                    if a_a then
                        ...
                        if (c_a == false) then
                            self.filter = nil; return cd, dd
                        end
                    end; _b.header = "Basalt Program Error " .. _c; _b.error(dd)
                end
    ...
    function bb:resume(_c, ...)
        ...
        if cc then
            ...
            if
                ...
                if _d then _d(self.program, cc, dc) end
            end
        else
            local _d = self.program.get("doneCallback")
            if _d then
            ...
            if ad then
                ...
                if (cd == false) then
                    self.filter = nil; return cc, dc
                end
            end; _b.header = "Basalt Program Error"
            _b.error(dc)
        end; return cc, dc
    end

    function bb:stop()
        if self.coroutine == nil or
            coroutine.status(self.coroutine) == "dead" then
            self.program.set("running", false)
            return
        end
    ...
    function ab:init(_c, ac)
        ...
        self:observe("width", function(bc, cc)
            ...
            if dc then dc:resize(cc, bc.get("height")) end
        end)
    ...
    function ab:execute(_c, ac, bc, ...)
        ...
        return self
    end; function ab:sendEvent(_c, ...)
        self:dispatchEvent(_c, ...)
        return self
    end; function ab:onError(_c)
        self.set("errorCallback", _c)
        return self
    end; function ab:onDone(_c)
...
project["elements/Tree.lua"] = function(...)
    ...
    ba.defineProperty(ba, "selectedColor", { default = colors.lightBlue, type = "color" })
    ba.defineEvent(ba, "mouse_click")
    ba.defineEvent(ba, "mouse_scroll")
    function ba.new()
        local da = setmetatable({}, ba):__init()
        da.class = ba; da.set("width", 30)
        da.set("height", 10)
        da.set("z", 5)
        return da
    end
    ...
    function ba:collapseNode(da)
        self.get("expandedNodes")[da] =
            nil; self:updateRender()
        return self
    end; function ba:toggleNode(da)
    ...
    local function ca(da, _b, ab, bb)
        bb = bb or {}
        ab =
            ab or 0; for cb, db in ipairs(da) do
            table.insert(bb, { node = db, level = ab })
            if
                _b[db] and db.children then
    ...
    function ba:render()
        ...
        for y = 1, _b do
            local _c = da[y + cb]
            if _c then
                local ac = _c.node; local bc = _c.level
                ...
                self:textFg(1, y,
                    bd .. string.rep(" ", self.get("width") - #bd), self.get("foreground"))
            else
...
project["elements/Image.lua"] = function(...)
    ...
    function _b.new()
        local cb = setmetatable({}, _b):__init()
        cb.class = _b; cb.set("width", 12)
        cb.set("height", 6)
        cb.set("background", colors.black)
        cb.set("z", 5)
        return cb
    end; function _b:init(cb, db)
        ca.init(self, cb, db)
        self.set("type", "Image")
        return self
    end
    ...
    function _b:getImageSize()
        ...
        if not cb[1] or not cb[1][1] then
            return 0, 0
        end; return #cb[1][1][1], #cb[1]
    end

    function _b:getPixelData(cb, db)
        local _c = self.get("bimg")[self.get("currentFrame")]
        if not _c or not _c[db] then return end; local ac = _c[db][1]
        local bc = _c[db][2]
        local cc = _c[db][3]
    ...
    local function ab(cb, db)
        ...
        if not _c[db] then _c[db] = { "", "", "" } end; return _c
    end
    local function bb(cb, db, _c)
        if not cb.get("autoResize") then return end
        local ac = cb.get("bimg")
        local bc = db; local cc = _c
        ...
        for dc, _d in ipairs(ac) do
            for y = 1, cc do
                if not _d[y] then _d[y] = { "", "", "" } end; local ad = _d[y]
                while #ad[1] <
                    bc do
                    ad[1] = ad[1] .. " "
                end; while #ad[2] < bc do
                    ad[2] = ad[2] .. "f"
                end; while #ad[3] < bc do ad[3] = ad[3] .. "0" end
            end
        end
    end
    function _b:setText(cb, db, _c)
        if
            type(_c) ~= "string" or #_c < 1 or cb < 1 or db < 1 then
            return self
        end
        if
            not self.get("autoResize") then
            local cc, dc = self:getImageSize()
            if db > dc then return self end
        end; local ac = ab(self, db)
        if self.get("autoResize") then
            bb(self, cb + #_c - 1, db)
        else
            local cc = #ac[db][1]
            if cb > cc then return self end
            _c = _c:sub(1, cc - cb + 1)
        end
        local bc = ac[db][1]
    ...
    function _b:getText(cb, db, _c)
        ...
        if not ac or not ac[db] then return "" end; local bc = ac[db][1]
        if not bc then
            return ""
        end
        if _c then return bc:sub(cb, cb + _c - 1) else return bc:sub(cb, cb) end
    end
    ...
    function _b:getFg(cb, db, _c)
        if not cb or not db then return "" end
        local ac = self.get("bimg")[self.get("currentFrame")]
        if not ac or not ac[db] then return "" end; local bc = ac[db][2]
        if not bc then
            return ""
        end
        if _c then return bc:sub(cb, cb + _c - 1) else return bc:sub(cb) end
    end
    ...
    function _b:getBg(cb, db, _c)
        if not cb or not db then return "" end
        local ac = self.get("bimg")[self.get("currentFrame")]
        if not ac or not ac[db] then return "" end; local bc = ac[db][3]
        if not bc then
            return ""
        end
        if _c then return bc:sub(cb, cb + _c - 1) else return bc:sub(cb) end
    ...
    function _b:setPixel(cb, db, _c, ac, bc)
        if _c then self:setText(cb, db, _c) end; if ac then
            self:setFg(cb, db, ac)
        end; if bc then self:setBg(cb, db, bc) end; return self
    end

    function _b:nextFrame()
        if not self.get("bimg").animation then return self end; local cb = self.get("bimg")
        local db = self.get("currentFrame")
        local _c = db + 1; if _c > #cb then _c = 1 end; self.set("currentFrame", _c)
        return self
    end
    ...
    function _b:addFrame()
        ...
        for y = 1, _c do ac[y] = { bc, cc, dc } end
        table.insert(cb, ac)
        return self
    end; function _b:updateFrame(cb, db)
        local _c = self.get("bimg")
        _c[cb] = db
        self:updateRender()
        return self
    end; function _b:getFrame(cb)
    ...
    function _b:getMetadata()
        local cb = {}
        local db = self.get("bimg")
        for _c, ac in pairs(db) do if (type(ac) == "string") then cb[_c] = ac end end; return cb
    end

    function _b:setMetadata(cb, db)
        if (type(cb) == "table") then
            for ac, bc in pairs(cb) do self:setMetadata(ac, bc) end; return self
        end
        local _c = self.get("bimg")
        if (type(db) == "string") then _c[cb] = db end; return self
    end

    function _b:render()
        ca.render(self)
        local cb = self.get("bimg")[self.get("currentFrame")]
        if not cb then return end; local db = self.get("offsetX")
        ...
        for y = 1, bc do
            ...
            if dc then
                ...
                if _d and ad and bd then
                    ...
                    if cd > 0 then
                        if db < 0 then
                            local dd = math.abs(db) + 1; _d = _d:sub(dd)
                            ad = ad:sub(dd)
                            bd = bd:sub(dd)
                        end; _d = _d:sub(1, cd)
                        ad = ad:sub(1, cd)
...
project["elements/Label.lua"] = function(...)
    ...
    function ca:init(da, _b)
        ...
        self.set("type", "Label")
        return self
    end; function ca:getWrappedText()
...
project["elements/Input.lua"] = function(...)
    ...
    aa.defineProperty(aa, "replaceChar", { default = nil, type = "string", canTriggerRender = true })
    aa.defineEvent(aa, "mouse_click")
    ...
    function aa:char(ba)
        ...
        if _b and #ca >= _b then return false end; if ab and not ba:match(ab) then
            return
                false
        end
    ...
    function aa:key(ba, ca)
        ...
        if ba == keys.left then
            if da > 1 then
                self.set("cursorPos", da - 1)
                if da - 1 <= ab then
                    self.set("viewOffset", math.max(0,
                        da - 2))
                end
            end
    ...
    function aa:mouse_click(ba, ca, da)
        if d.mouse_click(self, ba, ca, da) then
            ...
            local db = #bb + 1; local _c = math.min(db, cb + _b)
    ...
    function aa:focus()
        ...
        self:setCursor(self.get("cursorPos") -
            self.get("viewOffset"), 1, true, self.get("cursorColor") or self.get("foreground"))
        self:updateRender()
    end
    ...
    function aa:paste(ba)
        ...
        if
            _b and #bb > _b then
            bb = bb:sub(1, _b)
        end; if ab and not bb:match(ab) then
            return false
        end; self.set("text", bb)
    ...
    function aa:render()
        ...
        cb and ab or self.get("background")])
        if
            ...
            self:setCursor(
                self.get("cursorPos") - ca, 1, true, self.get("cursorColor") or self.get("foreground"))
        end
        local bc = ba:sub(ca + 1, ca + db)
        if ac and #ac > 0 then bc = ac:rep(#bc) end
...
project["elements/BarChart.lua"] = function(...)
    ...
    local da = require("libraries/colorHex")
    local _b = setmetatable({}, ca)
    _b.__index = _b; function _b.new()
        local ab = setmetatable({}, _b):__init()
        ab.class = _b; return ab
    end
...
project["elements/Button.lua"] = function(...)
    local _a = require("elementManager")
    local aa = _a.getElement("VisualElement")
    local ba = require("libraries/utils").getCenteredPosition; local ca = setmetatable({}, aa)
    ca.__index = ca
...
project["elements/BaseElement.lua"] = function(...)
    local _a = require("propertySystem")
    local aa = require("libraries/utils").uuid; local ba = require("errorManager")
    ...
    ca.defineProperty(ca, "type", {
        ...
        getter = function(da, _b, ab)
            if
                ab ~= nil and ab < 1 then
                return da._values.type
            end; return da._values.type[
            ab or 1]
        end
    })
    ...
    function ca.defineEvent(da, _b, ab)
        if not rawget(da, '_eventConfigs') then da._eventConfigs = {} end; da._eventConfigs[_b] = {
            requires = ab and ab or
                _b
        }
    end

    function ca.registerEventCallback(da, _b, ...)
        local ab = _b:match("^on") and _b or "on" .. _b; local bb = { ... }
        local cb = bb[1]
        da[ab] = function(db, ...)
    ...
    function ca:init(da, _b)
        ...
        self.basalt = _b; self._registeredEvents = {}
        ...
        while ab do
            if type(ab) == "table" and ab._eventConfigs then
                for cb, db in
                pairs(ab._eventConfigs) do if not bb[cb] then bb[cb] = db end end
            end
    ...
    function ca:postInit()
        if self._postInitialized then return self end
        self._postInitialized = true; if (self._props) then
            for da, _b in pairs(self._props) do self.set(da, _b) end
        end; self._props = nil; return self
    end; function ca:isType(da)
        for _b, ab in ipairs(self._values.type) do if ab == da then return true end end; return false
    end
    ...
    function ca:listenEvent(da, _b)
        ...
        if
            ...
            if _b then
                self._registeredEvents[da] = true; if self.parent then
    ...
    function ca:registerCallback(da, _b)
        ...
        if
            not self._values.eventCallbacks[da] then
    ...
    function ca:fireEvent(da, ...)
        if self.get("eventCallbacks")[da] then
            for _b, ab in
            ipairs(self.get("eventCallbacks")[da]) do
                local bb = ab(self, ...)
                return bb
            end
        end; return self
    ...
    function ca:dispatchEvent(da, ...)
        if self.get("enabled") == false then return false end; if self[da] then return self[da](self, ...) end; return
            self:handleEvent(da, ...)
    end; function ca:handleEvent(da, ...) return false end; function ca:onChange(da, _b)
        self:observe(da, _b)
        return self
    end
    ...
    function ca:getBaseFrame()
        if self.parent then
            return
                self.parent:getBaseFrame()
        end; return self
    end

    function ca:destroy()
        self._destroyed = true; self:removeAllObservers()
...
project["elements/TextBox.lua"] = function(...)
    ...
    _b.defineProperty(_b, "cursorColor", { default = nil, type = "color" })
    ...
    function _b.new()
        ...
        _c.class = _b; _c.set("width", 20)
        _c.set("height", 10)
        return _c
    end; function _b:init(_c, ac)
        ca.init(self, _c, ac)
        self.set("type", "TextBox")
        return self
    end; function _b:addSyntaxPattern(_c, ac)
        table.insert(self.get("syntaxPatterns"), { pattern = _c, color = ac })
        return self
    end
    ...
    function _b:char(_c)
        if
            not self.get("editable") or not self.get("focused") then
            return false
        end; ab(self, _c)
        return true
    end
    ...
    function _b:mouse_scroll(_c, ac, bc)
        if self:isInBounds(ac, bc) then
            ...
            local bd = math.max(0, math.min(ad,
                cc + _c))
            self.set("scrollY", bd)
            self:updateRender()
            return true
    ...
    function _b:mouse_click(_c, ac, bc)
        if ca.mouse_click(self, _c, ac, bc) then
            ...
            local bd = dc + ad; local cd = self.get("lines")
            if bd <= #cd then
                self.set("cursorY", bd)
    ...
    local function db(_c, ac)
        ...
        for _d, ad in ipairs(dc) do
            ...
            while true do
                ...
                cc = cc:sub(1, cd - 1) ..
                    string.rep(da[ad.color], dd - cd + 1) .. cc:sub(dd + 1)
                bd = dd + 1
            end
        end; return bc, cc
    end
    function _b:render()
        ...
        for y = 1, dc do
            local bd = y + bc; local cd = _c[bd] or ""
        ...
        if self.get("focused") then
            local bd = self.get("cursorX") - ac; local cd =
...
project["elements/Table.lua"] = function(...)
    ...
    function aa:addColumn(ba, ca)
        ...
        table.insert(da, { name = ba, width = ca })
        self.set("columns", da)
        return self
    end; function aa:addData(...)
    ...
    function aa:sortData(ba, ca)
        ...
        if not ca then
            ...
            table.sort(da, function(ab, bb) return ca(ab[ba], bb[ba]) end)
        end; return self
    end
                ...
                if _b >= bb and _b < bb + db.width then
                    if self.get("sortColumn") == cb then
                        self.set("sortDirection",
                            self.get("sortDirection") == "asc" and "desc" or "asc")
                    else
                        self.set("sortColumn", cb)
                        self.set("sortDirection", "asc")
                    end; self:sortData(cb)
                    break
                end; bb = bb + db.width
            end
        end
    ...
    function aa:render()
        ...
        for _c, ac in ipairs(ba) do
            local bc = ac.name; if _c == _b then
                bc = bc .. (
                    self.get("sortDirection") == "asc" and "\30" or "\31")
            end
            self:textFg(db, 1, bc:sub(1, ac.width), self.get("headerColor"))
            db = db + ac.width
        end
        for y = 2, bb do
            local _c = y - 2 + ab; local ac = ca[_c + 1]
            if ac and (_c + 1) <= #ca then
                ...
                for cc, dc in ipairs(ba) do
                    ...
                    local ad = _d ..
                        string.rep(" ", dc.width - #_d)
                    if cc < #ba then
                        ad =
                            string.sub(ad, 1, dc.width - 1) .. " "
                    end
...
project["log.lua"] = function(...)
    local aa = {}
    aa._logs = {}
    aa._enabled = false; aa._logToFile = false
    ...
    function aa.setLogToFile(ab) aa._logToFile = ab end
    ...
    local function _b(ab, ...)
        if
            not aa._enabled then
            return
        end; local bb = os.date("%H:%M:%S")
        ...
        for _d, ad in ipairs(table.pack(...)) do
            if _d > 1 then
                cc =
                    cc .. " "
            end; cc = cc .. tostring(ad)
        end; local dc = string.format("%s %s%s %s", bb, ac, bc, cc)
        da(dc)
        table.insert(aa._logs, { time = bb, level = ab, message = cc })
    end; function aa.debug(...) _b(aa.LEVEL.DEBUG, ...) end; function aa.info(...)
        _b(aa.LEVEL.INFO, ...)
    end

    function aa.warn(...) _b(aa.LEVEL.WARN, ...) end; function aa.error(...) _b(aa.LEVEL.ERROR, ...) end; return aa
end
project["elementManager.lua"] = function(...)
    ...
    if (bb == nil) then
        error("Unable to find directory " ..
            ab[2] .. " please report this bug to our discord.")
    end
    local db = require("log")
    local _c = package.path; local ac = "path;/path/?.lua;/path/?/init.lua;"
    ...
    if fs.exists(dc) then
        for ad, bd in ipairs(fs.list(dc)) do
            ...
            if cd then
                db.debug(
                    "Found element: " .. cd)
                cc._elements[cd] = { class = nil, plugins = {}, loaded = false }
            end
        end
    end; db.info("Loading plugins from " .. _d)
    if
        fs.exists(_d) then
            ...
            if cd then
                ...
                if
                    ...
                    for __a, a_a in pairs(dd) do
                        if (__a ~= "API") then
                            if (cc._plugins[__a] == nil) then cc._plugins[__a] = {} end; table.insert(cc._plugins[__a],
                                a_a)
                        else
                            cc._APIs[cd] = a_a
                        end
    ...
    if (minified) then
        ...
        pairs(minified_elementDirectory) do
            cc._elements[ad:gsub(".lua", "")] = { class = nil, plugins = {}, loaded = false }
        end; if
    ...
    function cc.loadElement(ad)
        if not cc._elements[ad].loaded then
            ...
            package.path = _c; cc._elements[ad] = { class = bd, plugins = bd.plugins, loaded = true }
            db.debug(
                "Loaded element: " .. ad)
            if (cc._plugins[ad] ~= nil) then
                for cd, dd in
                pairs(cc._plugins[ad]) do
                    if (dd.setup) then dd.setup(bd) end
                    if (dd.hooks) then
                        for __a, a_a in pairs(dd.hooks) do
                            local b_a = bd[__a]
                            if (type(b_a) ~= "function") then
                                error("Element " ..
                                    ad .. " does not have a method " .. __a)
                            end
                            ...
                            elseif (type(a_a) == "table") then
                                bd[__a] = function(c_a, ...)
                                    if a_a.pre then a_a.pre(c_a, ...) end
                                    local d_a = b_a(c_a, ...)
                                    if a_a.post then a_a.post(c_a, ...) end; return d_a
                                end
                            end
                        end
                    end; for __a, a_a in pairs(dd) do
                        if __a ~= "setup" and __a ~= "hooks" then bd[__a] = a_a end
                    end
                end
            end
        end
    ...
    function cc.getElement(ad)
        if not cc._elements[ad].loaded then
            cc.loadElement(ad)
        end; return cc._elements[ad].class
    end; function cc.getElementList() return cc._elements end; function cc.getAPI(ad)
        return cc._APIs[ad]
    end; return cc
end
project["propertySystem.lua"] = function(...)
    ...
    function _b.defineProperty(cb, db, _c)
        ...
        "get" .. ac] = function(bc, ...)
            ...
            if type(cc) ==
                "function" and _c.type ~= "function" then
                cc = cc(bc)
            end
        ...
        cb["set" .. ac] = function(bc, cc, ...)
            ...
            if
                ...
                if _c.type == "table" then
                    if cc == nil then
                        if not _c.allowNil then ca(2, cc, _c.type) end
                    end
                else
                    ca(2, cc, _c.type)
                end
    ...
    function _b.combineProperties(cb, db, ...)
        ...
        cb["get" .. ac] = function(bc)
            ca(1, bc, "element")
            local cc = {}
            for dc, _d in pairs(_c) do table.insert(cc, bc.get(_d)) end; return table.unpack(cc)
        end
    ...
    function _b.blueprint(cb, db, _c, ac)
        if not ab[cb] then
            ...
            cc.loaded = function(_d, ad)
                _d.loadedCallback = ad; return cc
            end
            cc.create = function(_d)
                ...
                for bd, cd in pairs(_d._values) do
                    ad._values[bd] = cd
                end; for bd, cd in pairs(_d._events) do
                    for dd, __a in ipairs(cd) do ad[bd](ad, __a) end
                end
                if (ac ~= nil) then ac:addChild(ad) end; ad:updateRender()
        ...
        end; local bc = { _values = {}, _events = {}, loadedCallback = function() end }
        bc.get = function(cc)
            local dc = bc._values[cc]
            local _d = cb._properties[cc]
            if
                type(dc) == "function" and _d.type ~= "function" then
                dc = dc(bc)
            end; return dc
        end
        bc.set = function(cc, dc)
            bc._values[cc] = dc; return bc
        end
        setmetatable(bc, {
            __index = function(cc, dc)
                ...
                if dc:match("^get%u") then
                    ...
                    return function() return cc._values[_d] end
                end; if dc:match("^set%u") then
                    local _d = dc:sub(4, 4):lower() .. dc:sub(5)
                    return function(ad, bd)
                        cc._values[_d] = bd; return cc
                    end
                end
                return ab[cb][dc]
            end
    ...
    function _b:__init()
        ...
        self.set = function(bc, cc, ...)
            ...
            if (_d ~= nil) then
                ...
                if _d.canTriggerRender then self:updateRender() end; self._values[bc] = bb(self, bc, cc, _d)
                if
                    dc ~= cc and self._observers[bc] then
                    for ad, bd in ipairs(self._observers[bc]) do bd(self, cc, dc) end
                end
            end
        end
        self.get = function(bc, ...)
            ...
            if
                ...
                return
            end; if type(cc) == "function" and dc.type ~= "function" then
                cc = cc(self)
            end; return
                dc.getter and dc.getter(self, cc, ...) or cc
        end; local cb = {}
        local db = getmetatable(self).__index
        while db do
            if
                rawget(db, '_properties') then
                for bc, cc in pairs(db._properties) do if not cb[bc] then cb[bc] = cc end end
            end; db = getmetatable(db) and
                rawget(getmetatable(db), '__index')
        end; self._properties = cb; local _c = getmetatable(self)
        local ac = _c.__index
        setmetatable(self, {
            __index = function(bc, cc)
                ...
                if dc then
                    ...
                    if
                        type(_d) == "function" and dc.type ~= "function" then
                        _d = _d(self)
                    end; return _d
                end
                if type(ac) == "function" then return ac(bc, cc) else return ac[cc] end
            end,
            __newindex = function(bc, cc, dc)
                local _d = self._properties[cc]
                if _d then
                    if _d.setter then dc = _d.setter(self, dc) end
                    dc = bb(self, cc, dc, _d)
            ...
            __tostring = function(bc)
                return
                    string.format("Object: %s (id: %s)", bc._values.type, bc.id)
            end
        })
        ...
        if self._observers[cb] then
            for _c, ac in ipairs(self._observers[cb]) do
                if ac == db then
                    ...
                    if #self._observers[cb] == 0 then self._observers[cb] = nil end; break
                end
            end
        end; return self
    end; function _b:removeAllObservers(cb)
        if cb then self._observers[cb] = nil else self._observers = {} end; return self
    end

    function _b:instanceProperty(cb, db)
        _b.defineProperty(self, cb, db)
        self._values[cb] = db.default; return self
    end

    function _b:removeProperty(cb)
        self._values[cb] = nil; self._properties[cb] = nil; self._observers[cb] =
            nil
        local db = cb:sub(1, 1):upper() .. cb:sub(2)
        self["get" .. db] = nil; self["set" .. db] = nil; return self
    end

    function _b:getPropertyConfig(cb) return self._properties[cb] end; return _b
end
project["init.lua"] = function(...)
    ...
    local function db(bc)
        ...
        package.path = ab; cc.header = "Basalt Loading Error"
        cc.error(bc)
    end; local _c, ac = pcall(require, "main")
    package.loaded.log = nil
    package.path = ab; if not _c then db(ac) else return ac end
end
project["errorManager.lua"] = function(...)
    local d = require("log")
    local _a = { tracebackEnabled = true, header = "Basalt Error" }
    ...
    function _a.error(ba)
        ...
        print()
        local ca = 2; local da; while true do
            local db = debug.getinfo(ca, "Sl")
            if not db then break end; da = db; ca = ca + 1
        end; local _b = da or
            debug.getinfo(2, "Sl")
        ...
        if ab and bb then
            ...
            if db then
                ...
                until not _c; db.close()
            end
        end; term.setBackgroundColor(colors.black)
        d.error(ba)
        _a.errorHandled = true; error()
    end; return _a
end
